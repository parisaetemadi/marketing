...
A publishing tool is a different thing from a reply bot, and one lived here
briefly: a Pinterest pin generator and poster, removed in August 2026 when the
account was closed. The distinction it rested on is worth keeping if the
question comes up again — a reply, a DM and a cold email all arrive at somebody
who did not ask for them, while a pin or a guide goes into an index and waits
to be found. Anything that cannot be described the second way belongs on the
manual side of this line, whatever else it has going for it.

That tool also carried its segment's tone rules as build failures rather than
comments: no urgency vocabulary, no exclamation marks, no price in a headline
for the `bereaved` segment, and no figure that did not appear on the page it
linked to. If anything ever publishes on this product's behalf again, it needs
the same treatment. The code is in this repo's history.
